...
class Button():
	...
	def unclick(self):
		self.msg_image = self.font.render(generator_types[self.type], True, self.text_color[0], self.button_color)
# этот класс представляет кнопку на игровом экране.
#  Каждая кнопка имеет тип и может быть нажата игроком. Когда кнопка нажата, она подсвечивается, и алгоритм генерации игрового лабиринта меняется на тип кнопки.
class Game():
	def __init__(self):   # это конструктор класса Game. Он инициализирует библиотеку Pygame, создает экран игры, создает объект Map и создает список объектов Button.
		pygame.init()
...
# этот класс представляет саму игру. Он содержит состояние игры, включая карту, текущий режим (либо создание лабиринта, либо его решение) и 
# текущий алгоритм создания лабиринта. Класс Game также имеет список объектов Button, 
# которые отображаются на экране игры. Класс Game имеет метод play, который обновляет и рисует состояние игры в каждом кадре.


	def play(self):    # это основной игровой цикл, который обновляет и рисует состояние игры в каждом кадре. Цикл обрабатывает ввод игрока, генерирует и решает лабиринты, а также обновляет игровой экран.
		self.clock.tick(30)
...
#Игра инициализируется путем создания нового объекта Game и вызова его метода play. Этот метод сначала обновляет экран, 
# рисуя кнопки и ячейки лабиринта, а затем ожидает ввода данных пользователем. Если пользователь нажимает на одну из кнопок, игровая переменная
#  maze_type обновляется и создается новый лабиринт.
#  Если пользователь перемещает персонажа с помощью клавиш со стрелками, позиция персонажа обновляется, и экран перерисовывается, чтобы отразить изменение.			